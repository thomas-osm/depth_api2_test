 ...
 *     David Orme - Ported SWT implementation to draw2d
 *******************************************************************************/
...
/**
 * This is a port of SWT's FormLayout to draw2d.
 * <p>
...
public class FormLayout implements LayoutManager {
	...
	public Dimension getPreferredSize(IFigure container, int wHint, int hHint) {
		return computeSize(container, wHint, hHint, true);
	}
	 ...
	 */
	int computeHeight (IFigure control, FormData data, boolean flushCache) {
	...
	protected Dimension computeSize (IFigure composite, int wHint, int hHint, boolean flushCache) {
		Dimension size = layout (composite, false, 0, 0, wHint, hHint, flushCache);
		if (wHint != SWT.DEFAULT) size.width = wHint;
		if (hHint != SWT.DEFAULT) size.height = hHint;
		return size;
	}

	protected boolean flushCache (IFigure control) {
		Object data = getLayoutData (control);
	 ...
	 * respect to the preferred height of the control.
	 */
	int computeWidth (IFigure control, FormData data, boolean flushCache) {
	...
	protected void layout (IFigure composite, boolean flushCache) {
	...
	Object getLayoutData(IFigure control) {
		return control.getParent().getLayoutManager().getConstraint(control);
	}
	
	void setLayoutData(IFigure control, Object constraint) {
		control.getParent().getLayoutManager().setConstraint(control, constraint);
	}
	
	Dimension layout (IFigure composite, boolean move, int x, int y, int width, int height, boolean flushCache) {
		List childList = composite.getChildren();
		IFigure [] children = (IFigure[]) childList.toArray(new IFigure[childList.size()]);
		for (int i=0; i<children.length; i++) {
			IFigure child = children [i];
			FormData data = (FormData) getLayoutData (child);
			if (data == null) setLayoutData (child, data = new FormData ());
		...
		for (int i=0; i<children.length; i++) {
			...
			if (width != SWT.DEFAULT) {
				...
				if (data.height == SWT.DEFAULT && !data.needed) {
					...
					int currentWidth = Math.max (0, x2 - x1 - insets.left - insets.right);
