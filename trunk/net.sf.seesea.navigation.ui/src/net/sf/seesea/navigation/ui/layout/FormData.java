...
void computeSize (IFigure control, int wHint, int hHint, boolean flushCache) {
	...
	if (wHint == this.width && hHint == this.height) {
		if (defaultWidth == -1 || defaultHeight == -1 || wHint != defaultWhint || hHint != defaultHhint) {
			Dimension size =  control.getPreferredSize(wHint, hHint);
			defaultWhint = wHint;
			defaultHhint = hHint;
...
int getWidth (IFigure control, boolean flushCache) {
	needed = true;
	computeSize (control, width, height, flushCache);
	return cacheWidth;
}

int getHeight (IFigure control, boolean flushCache) {
	computeSize (control, width, height, flushCache);
	return cacheHeight;
}

FormAttachment getBottomAttachment (IFigure control, int spacing, boolean flushCache) {
	...
	IFigure bottomControl = bottom.control;
	...
	isVisited = true;
	FormData bottomData = (FormData) bottomControl.getParent().getLayoutManager().getConstraint(bottomControl);
...
FormAttachment getLeftAttachment (IFigure control, int spacing, boolean flushCache) {
	...
	IFigure leftControl = left.control;
	if (leftControl != null) {
	...
	isVisited = true;
	FormData leftData = (FormData) leftControl.getParent().getLayoutManager().getConstraint(leftControl);
...
FormAttachment getRightAttachment (IFigure control, int spacing, boolean flushCache) {
	...
	IFigure rightControl = right.control;
	...
	isVisited = true;
	FormData rightData = (FormData) rightControl.getParent().getLayoutManager().getConstraint(rightControl);
...
FormAttachment getTopAttachment (IFigure control, int spacing, boolean flushCache) {
	...
	IFigure topControl = top.control;
	if (topControl != null) {
	...
	isVisited = true;
	FormData topData = (FormData) topControl.getParent().getLayoutManager().getConstraint(topControl);
